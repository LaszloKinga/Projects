...
char_to_idx = torch.load("char_to_idx_v2.pt",weights_only=True)
idx_to_char = torch.load("idx_to_char_v2.pt",weights_only=True) 
max_len = 20 

model = LSTMWordPredictor(input_size=len(char_to_idx), hidden_size=256, output_size=len(char_to_idx))
model.load_state_dict(torch.load("lstm_word_predictor_v2.pt", weights_only=True))
model.eval()
